...
class VideoClip(NamedObject):
	def __init__(self, reader):
		...
		if self.version[0] > 2017 or (self.version[0] == 2017 and self.version[1] >= 2):  # 2017.2 and up
		...
		if source:
			resource_reader = ResourceReader(source, self.assets_file, offset, size)
		else:
			resource_reader = ResourceReader(self.reader, self.reader.Position, size)
		self.VideoData = resource_reader.get_data()
