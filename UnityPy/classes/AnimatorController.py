...
class BlendTreeNodeConstant:
	def __init__(self, reader):
		...
		if (version[0] > 4
			...
			or (version[0] == 4 and version[1] == 1 and version[2] >= 3)):  # 4.1.3 and up
...
class StateConstant:
	def __init__(self, reader):
		...
		self.m_TransitionConstantArray = [
			TransitionConstant(reader)
			for _ in range(numTransistions)
		]
		...
		if version[0] < 5 or (version[0] == 5 and version[1] < 2):  # 5.2 down
			...
			self.m_LeafInfoArray = [
				LeafInfoConstant(reader)
				for _ in range(numInfos)
			]

		numBlends = reader.read_int()
		self.m_BlendTreeConstantArray = [
			BlendTreeConstant(reader)
			for _ in range(numBlends)
		]
...
class SelectorStateConstant:
	def __init__(self, reader):
		...
		self.m_TransitionConstantArray = [
			SelectorTransitionConstant(reader)
			for _ in range(numTransitions)
		]
		self.m_FullPathID = reader.read_u_int()
...
class ControllerConstant:
	def __init__(self, reader):
		...
		self.m_LayerArray = [
			LayerConstant(reader)
			for _ in range(numLayers)
		]

		numStates = reader.read_int()
		self.m_StateMachineArray = [
			StateMachineConstant(reader)
			for _ in range(numStates)
		]
