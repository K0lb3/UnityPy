...
class VertexData:
	...
	def FixChannel(self):
		if any([x.dimension > 4 for x in self.m_Channels]):  # m_Channels.FirstOrDefault(x => x.dimension > 4) != null: #
	...
	def __init__(self, reader):
		...
		if version[0] > 2 or (version[0] == 2 and version[1] >= 6):  # 2.6.0 and up
			...
			if ((version[0] > 2017 or (version[0] == 2017 and version[1] >= 4))  # 2017.4
					or ((version[0] == 2017 and version[1] == 3 and version[2] == 1) and self.build_type.IsPatch)  # fixed after 2017.3.1px
					or ((version[0] == 2017 and version[1] == 3) and self.m_MeshCompression == 0)):  # 2017.3.xfx with no compression
	...
	def ProcessData(self):
		if hasattr(self, 'm_StreamData') and self.m_StreamData.get('path'):
			if self.m_VertexData.m_VertexCount > 0:  #
				resourceReader = ResourceReader(self.m_StreamData.path, self.assets_file, self.m_StreamData.offset, self.m_StreamData.size)
				...
				if channelMask[chn]:
					...
					for v in range(m_VertexCount):
						...
						for d in range(m_Channel.dimension):
							...
							componentBytes[dstOffset:dstOffset + componentByteSize] += self.m_VertexData.m_DataSize[componentOffset:componentOffset + componentByteSize]
	...
	def BytesToFloatArray(inputBytes, size):
		...
		for i in range(math.ceil(len(inputBytes) / size)):
			...
			if size == 1:
				...
				value = float.from_bytes(inputBytes[i * 2:i * 2 + 2], byteorder = 'big', signed = True)  # return Half.ToHalf((ushort)BitConverter.ToInt16(value, startIndex));
			elif size == 4:
